...
const LuckyPick = () => {
  ...
  const uploadToCloudinary = async (file) => {
    ...
    formData.append("upload_preset", "lucky_pick_preset"); // Make sure this is your actual Cloudinary preset
  
    try {
      const response = await fetch("https://api.cloudinary.com/v1_1/dokhvz05z/image/upload", {
        method: "POST",
        body: formData,
      });
      ...
      return data.secure_url; // Cloudinary's secure URL
    } catch (error) {
  ...
  return (
    <Container className="luckyPick-container">
      ...
      <Row className="img-header-row text-center">
        <Col>
        <img src={luckyPickLogo} alt="Lucky Pick Logo" className="headerImage" />
        </Col>
        </Row>
      ...
      <Col md={6}>
          <Form.Group controlId="beforePhoto">
            ...
            {previewBefore && (
              <Image src={previewBefore} thumbnail className="submissionImage mt-2" />
            )}
          </Form.Group>
        </Col>
        <Col md={6}>
          <Form.Group  controlId="afterPhoto">
            ...
            {previewAfter && (
              <Image src={previewAfter} thumbnail className="submissionImage mt-2 justify-content-center text-center" />
            )}
          </Form.Group>
        </Col>
      </Row>
    ...
    <h5>
      ...
      {submissions.length > 0
        ? (
            submissions.reduce(
              (acc, sub) => {
                acc.cashStart += parseFloat(sub.cashStart) || 0;
                acc.cashEnd += parseFloat(sub.cashEnd) || 0;
                return acc;
              },
  ...
  <div className="mt-2">
    <table className="table table-striped mt-5">
      ...
      <tbody>
        ...
        <tr>
          <td><strong>Free Games:</strong></td>
          <td>{sub.freeGames}</td>
        </tr>
        <tr>
          <td><strong>Numbers Off Board:</strong></td>
          <td>{sub.numbersOffBoard}</td>
        </tr>
        <tr>
          <td><strong>Wilds:</strong></td>
          <td>{sub.wilds}</td>
        </tr>
        <tr>
        ...
        <tr>
          <td><strong>Hit Progressive:</strong></td>
          <td>{sub.hitProgressive ? "Yes" : "No"}</td>
        </tr>
        <tr>
          <td><strong>Stages Hit:</strong></td>
